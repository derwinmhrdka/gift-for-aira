...
export default function BankTransferSection() {
  ...
  return (
    <section
      ...
      aria-labelledby="transfer-bank-heading"
    >
      <div className="mx-auto max-w-lg px-0 text-center sm:max-w-xl lg:max-w-2xl">
        <h2
          id="transfer-bank-heading"
          className="font-display text-2xl font-bold text-stone-800 sm:text-3xl"
        >
          Transfer Bank
        </h2>
      ...
      <div className="mx-auto mt-8 w-full max-w-md px-0 sm:mt-10 sm:max-w-lg">
        <div className="relative aspect-[85.6/53.98] w-full overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-5 shadow-2xl ring-1 ring-white/10 sm:p-6 md:p-8">
          {/* Mastercard-style circles (decorative, not a trademark logo) */}
          <div
            className="pointer-events-none absolute -right-4 top-6 flex items-center gap-0 sm:top-8 sm:right-2"
            aria-hidden
          >
            <span className="h-14 w-14 rounded-full bg-red-500/95 opacity-95 sm:h-16 sm:w-16" />
            <span className="-ml-7 h-14 w-14 rounded-full bg-amber-400/95 opacity-95 mix-blend-screen sm:-ml-8 sm:h-16 sm:w-16" />
          </div>

          <div className="pointer-events-none absolute left-5 top-5 h-9 w-12 rounded-md bg-gradient-to-br from-amber-200/90 to-amber-600/80 shadow-inner sm:left-6 sm:top-6 sm:h-10 sm:w-14" />
        ...
        <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:justify-center">
          <button
            ...
            onClick={copy}
            disabled={!configured}
            className="font-display touch-manipulation rounded-3xl bg-violet-600 px-6 py-3.5 text-base font-bold text-white shadow-md transition hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-2 focus:ring-offset-aira-yellow disabled:cursor-not-allowed disabled:opacity-50 sm:min-w-[200px] sm:py-3"
          >
