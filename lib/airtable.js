...
/**
 * Fetch participation rows for a hashed visitor id.
 */
export async function getParticipationRecords(visitorHash) {
  const hash = String(visitorHash ?? "").trim();
  if (!hash) return [];

  const { apiKey, url } = participationTableUrl();
  const formula = `{${PARTICIPATION_FIELD_MAP.visitorHash}} = '${hash.replace(/'/g, "\\'")}'`;
...
export async function getParticipatedGameSet(visitorHash) {
  const records = await getParticipationRecords(visitorHash);
  return new Set(records.map((record) => record.game).filter(Boolean));
}

export async function hasParticipatedInGame(visitorHash, game) {
  const participated = await getParticipatedGameSet(visitorHash);
  return participated.has(game);
}

/**
 * Record participation. Idempotent per visitor + game.
 */
export async function recordParticipation({ visitorHash, game, wishId }) {
  const hash = String(visitorHash ?? "").trim();
  const gameId = String(game ?? "").trim();
  if (!hash || !gameId) {
    throw new Error("Visitor hash and game are required.");
  }

  if (await hasParticipatedInGame(hash, gameId)) {
    return { alreadyRecorded: true };
  }

  const { apiKey, url } = participationTableUrl();
  const fields = {
    [PARTICIPATION_FIELD_MAP.visitorHash]: hash,
