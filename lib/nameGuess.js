...
export function wrongAttemptMask(guessPart, targetName, alreadyRevealed) {
  const g = normalizeName(guessPart);
  const t = normalizeName(targetName);

  return targetName.split("").map((_, index) => {
    if (alreadyRevealed[index]) return false;
    if (index >= g.length) return false;
    return g[index] !== t[index];
  });
}

export function mergeRevealedFromGuess(guessPart, targetName, alreadyRevealed) {
  const g = normalizeName(guessPart);
  const t = normalizeName(targetName);

  return targetName.split("").map((_, index) => {
    if (alreadyRevealed[index]) return true;
    return index < g.length && g[index] === t[index];
  });
}

export function allLettersRevealed(revealed, targetName) {
  return (
    revealed.length === targetName.length && revealed.every(Boolean)
  );
}
