...
async function scanAirtableCounts() {
...
async function getAirtableCounts({ force = false } = {}) {
  ...
  try {
    const counts = await scanAirtableCounts();
...
export async function getReactionsSince(sinceMs = 0, { includeEvents = true } = {}) {
  const memoryCounts = getMemoryCounts();
  const wantEvents = includeEvents && sinceMs > 0;

  if (!airtableConfigured()) {
    return {
      counts: memoryCounts,
      events: wantEvents ? getMemoryEventsSince(sinceMs) : [],
      source: "memory",
    };
  }
  ...
  try {
    const [airtableCounts, events] = await Promise.all([
      getAirtableCounts(),
      wantEvents ? getAirtableEventsSince(sinceMs) : Promise.resolve([]),
    ]);
