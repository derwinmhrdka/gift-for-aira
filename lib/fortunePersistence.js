...
const FORTUNE_TIMEZONE = "Asia/Jakarta";

/** Tanggal kalender WIB — ramalan reset otomatis setiap hari baru. */
...
function pruneOldFortuneEntries(visitorId, todayKey = getFortuneDateKey()) {
  ...
  for (let i = 0; i < localStorage.length; i += 1) {
    ...
    const dateKey = key.slice(prefix.length).split(":").pop();
    if (dateKey && dateKey !== todayKey) keysToRemove.push(key);
  }
...
export async function resolveFortuneForVisitor(
  ...
  if (!isAdmin) {
    pruneOldFortuneEntries(visitorId, dateKey);
